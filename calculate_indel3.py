...
def calc_mle(total_reads, indel_counts, background):
    indel_range = np.array(list(range(indel_counts)))
    ...
    with open(output_file, 'w') as out_handle:
        ...
        for i, l in enumerate(output_text):
            if i in controls:
                ...
                out_handle.write(f'{l},{mle_percentage},{lower_bound},{upper_bound}\n')
...
def whole_file_read(sample_sheet, file_type, output_file, hash_flag, mle, verbose, quiet):
    ...
    '''
    if not quiet:
        print(f'Reading input sheet from {sample_sheet}')
    if mle:
        mle_string = ''
    else:
    ...
    with open(sample_sheet) as in_handle, open(output_file, 'w') as out_handle:
        out_handle.write(
            f'sample,perfect matches,indels,misaligned reads,reads below threshold, reads with miscalled bases, reads with replacements,indel percentage, notes{mle_string}\n')
        for l in in_handle:
            sample_name, file_name, guide, target = l.strip().split(',')[:4]
            if verbose:
                print(f'Analyzing sample {sample_name} from {file_name}')
            ...
            out_handle.write(f'{sample_name},{total_list_string},{indel_rate},{note}\n')
    if not mle:
